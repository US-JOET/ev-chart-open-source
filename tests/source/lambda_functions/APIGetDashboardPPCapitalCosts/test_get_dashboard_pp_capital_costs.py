...
from APIGetDashboardPPCapitalCosts.index import handler as api_get_dashboard_program_performance
...
@patch("APIGetDashboardPPCapitalCosts.index.aurora")
def test_valid_200_as_jo(
    ...
    assert "unique_nevi_stations"in response_body
    assert "capital_cost_metrics_available"in response_body
    assert "average_nevi_capital_cost" in response_body
    assert "federal_funding" in response_body
    assert "capital_costs_total_nevi" in response_body
    assert "nonfederal_funding" in response_body
    assert "deployment_cost" in response_body
    assert "capital_cost_stations_count" in response_body
    assert "capital_cost_ports_count" in response_body
